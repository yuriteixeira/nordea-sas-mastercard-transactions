...
async function filterTransactionsNotSent(ynabApi, transactions, budgetId, accountId, startDate) {
  const filteredTransactions = filterTransactionsSinceStartDate(transactions, startDate)
  const earliestDateUtc = filteredTransactions.reduce(getEarliestDate, new Date())
  ...
  return filteredTransactions.filter(transaction => !isTransactionAlreadyInYnab(transaction, ynabTransactions))
}
